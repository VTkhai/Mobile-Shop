...
{
    public class From8To12MilStrategy : IPriceRangeStrategy
    {
        public IQueryable<Product> FilterProducts(MobileShopOnlineEntities db, int categoryId)
        {
            var products = db.Products.OrderByDescending(p => p.ProductID).Where(p => p.Price > 8000000 && p.Price <= 12000000);
            if (categoryId != 0)
